...
import { calculateScore, getSelectedScore, isFarkle, canContinue } from "../utils/scoreCalculator";
...
function rollDice(): DiceValue[] {
    return Array.from({ length: 6 }, () => (Math.floor(Math.random() * 6) + 1) as DiceValue);
}
...
function gameReducer(state: GameState, action: GameAction): GameState {
    switch (action.type) {
        case "ROLL_DICE": {
            ...
            const availableCombinations = calculateScore(newDice);
        ...
        case "BANK_SCORE": {
            ...
            const newTotalScore = state.players[state.currentPlayer].totalScore + state.turnScore + selectedScore;
...
export function useFarkleGame() {
    ...
    const currentTurnScore = getSelectedScore(gameState.dice, gameState.selectedDice);
    const canBank = currentTurnScore > 0;
    const canRoll = gameState.gamePhase === "rolling";
    const isFarkled = isFarkle(gameState.dice) && gameState.gamePhase === "rolling" && gameState.rollsInTurn > 0;
