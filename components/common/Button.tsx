...
// LightsOut IconButton
export const LightsOutIconButton = forwardRef<
  HTMLButtonElement,
  ButtonProps & { "aria-label": string }
>(({ children, icon, ...props }, ref) => (
  <LightsOutButton ref={ref} {...props} icon={icon || children}>
    {!icon && children && (
      <span className="sr-only">{props["aria-label"]}</span>
    )}
  </LightsOutButton>
));
LightsOutIconButton.displayName = "LightsOutIconButton";
