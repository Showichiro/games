...
// TypeScript types
export type ButtonVariant = 'primary' | 'secondary' | 'success' | 'danger' | 'info' | 'light' | 'dark' | 'ghost'; // Added ghost, removed warning, link
...
const Button = forwardRef<HTMLButtonElement, ButtonProps>(
  (
    ...
    const baseClasses = 'font-semibold focus:outline-none focus:ring-2 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed transition-colors duration-150 ease-in-out';
    ...
    const variantClasses: Record<ButtonVariant, string> = {
      ...
      ghost: 'bg-transparent hover:bg-neutral-100 dark:hover:bg-neutral-700 text-neutral-700 dark:text-neutral-200 focus:ring-blue-500 focus:ring-offset-0',
    };
    ...
    const fullWidthClasses = fullWidth ? 'w-full flex justify-center items-center' : 'inline-flex items-center';
    ...
    const iconSpacingClass = children ? (iconPosition === 'left' ? 'mr-2' : 'ml-2') : '';
      ...
      >
        ...
        <span className={isLoading ? 'invisible' : 'visible flex items-center'}>
          {icon && iconPosition === 'left' && (
            <span className={twMerge(iconSpacingClass, iconSizeClasses[size])}>{icon}</span>
          )}
          {children}
...
export const IconButton = forwardRef<HTMLButtonElement, ButtonProps & { 'aria-label': string }>(({ children, icon, ...props }, ref) => (
  <Button ref={ref} {...props} icon={icon || children}>
    {!icon && children && <span className="sr-only">{props['aria-label']}</span>}
  </Button>
));
...
const lightsOutBaseClasses = 'transition-all duration-300 ease-out relative overflow-hidden';
const lightsOutHoverClasses = 'hover:shadow-[0_0_30px_-5px_rgba(0,0,0,0.3)]'; // Example hover effect

export const LightsOutButton = forwardRef<HTMLButtonElement, ButtonProps>(({className, motionProps, ...props }, ref) => {
  ...
  return (
    <Button
      ref={ref}
      className={twMerge(lightsOutBaseClasses, lightsOutHoverClasses, className)}
...
export const LightsOutPrimaryButton = forwardRef<HTMLButtonElement, Omit<ButtonProps, 'variant'>>((props, ref) => (
  <LightsOutButton ref={ref} variant="primary" {...props} />
));
LightsOutPrimaryButton.displayName = 'LightsOutPrimaryButton';

export const LightsOutSecondaryButton = forwardRef<HTMLButtonElement, Omit<ButtonProps, 'variant'>>((props, ref) => (
  <LightsOutButton ref={ref} variant="secondary" {...props} />
));
LightsOutSecondaryButton.displayName = 'LightsOutSecondaryButton';

export const LightsOutSuccessButton = forwardRef<HTMLButtonElement, Omit<ButtonProps, 'variant'>>((props, ref) => (
    <LightsOutButton ref={ref} variant="success" {...props} />
));
